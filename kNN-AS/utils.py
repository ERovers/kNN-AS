...
def plot_potential_start(potential, xlim, ylim, start):
    '''
    Plots potential based on analytic function.
    '''
    colors = ['blue', 'cyan', 'green', 'yellow', 'red', 'white']  # Define your color transitions
    cmap = LinearSegmentedColormap.from_list("custom_jet", colors)

    jet_colors = plt.get_cmap('jet')(np.linspace(0, 1, 256))
    new_color = np.array([1, 1, 1, 1])
    new_colors = np.append(jet_colors, new_color).reshape(257,4)
    cjet = LinearSegmentedColormap.from_list('cjet', new_colors)
    ...
    Z = potential(X, Y)  # evaluation of the function on the grid
    Z = np.flip(Z,0)
    im = plt.imshow(Z, cmap=cjet, extent=[xlim[0], xlim[1], ylim[0], ylim[1]])  # drawing the function
    cset = plt.contour(Z, levels=[-0.5,0], colors='black', linewidths=5, extent=[xlim[0], xlim[1], ylim[1], ylim[0]], linestyles='solid')
    plt.colorbar(im)  # adding the colobar on the right
    plt.scatter(start[:,0], start[:,1], marker="*", s=200, facecolors='white', edgecolors='black', linewidths=1.5)
    plt.show()
...
def area_explored(potential, xlim, ylim, data, threshold):
    ...
    X, Y = np.meshgrid(x, y)  # grid of point
    Z = np.flip(potential(X, Y),0)  # evaluation of the function on the grid
