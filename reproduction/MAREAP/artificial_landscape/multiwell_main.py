...
from REAP_MA2 import run_trial

TASK_ID = 0 #int(sys.argv[1]) - 1 # From SGE (we must subtract 1 because it starts at 1 instead of 0) --> for parallel runs in clusters
trials_per_task = int(sys.argv[2]) # For parallel runs in clusters
...
kwargs = {
    ...
    'xlim': (-3, 3, 0.1),  # Limits where to compute area covered by agents
    'ylim': (-3, 3, 0.1),  # Limits where to compute area covered by agents
    'threshold': -60,  # Limits where to compute area covered by agents
    'debug': 1,  # Set to 1 to get plots of the landscape after a certain number of iterations
    'potential_func': potential_with_smooth_minima_func,  # Change depending on potential used
...
for i in range(int(sys.argv[1]),trials_per_task):
    run_trial(
        potential=potential_with_smooth_minima,  # Change according to desired potential
        initial_positions=[[1.6, 0.4 , 0], [-1.2, -1.2, 0]],
        # Change according to number of agents and their starting coordinates
        epochs=400,  # Change # of epochs as needed
        output_dir='testing_smooth_epoch400',  # Change output directory as needed
