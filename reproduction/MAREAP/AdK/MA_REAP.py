...
def run_trajectory(system, topology, integrator, positions, output_file, n_steps=1000, platform='CUDA'):
    ...
    sys.stdout.write("start minimization\n")
...
def spawn_trajectories(initial_positions, top_file, n_steps=1000, output_files=[]):
    ...
    for positions, output_file in zip(initial_positions, output_files):
        ...
        while True:
            try:
                sys.stdout.write(f"Start simulation {output_file}\n")
                run_trajectory(system, topology, integrator, pos, output_file, n_steps=n_steps)
            except Exception as e:
                sys.stdout.write(f"Error occurred: {e}\n")
                sys.stdout.write(f"Restarting simulation\n")
                pos = random.choice(initial_positions)
...
def collect_initial_data(num_trajectories, traj_len, initial_positions, output_dir, output_prefix, top_file):
    ...
    for a in range(n_agents):
        for t in range(num_trajectories):
            ...
            data = project_angles(fname, top_file)
...
def run_trial(initial_structures, epochs, output_dir='', output_prefix='', **kwargs):
    ...
    **kwargs: used to specify model hyperparameters. Must include following keys:
    ...
    for e in range(epochs):
        print("Running epoch: {}/{}".format(e + 1, epochs), flush=True)
        print("\n", flush=True)
        tmp = [i for a in trajectories for i in a]
        traj_total.append(tmp)
        # Clustering
    ...
    ### Save results ###
    np.save(os.path.join(output_dir, output_prefix + 'trajectories.npy'), [x for xs in traj_total for x in xs])
